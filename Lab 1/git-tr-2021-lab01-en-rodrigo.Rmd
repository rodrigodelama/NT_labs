...
test <- seq(1, 10, 0.5); test
```

Accessing values on a sequence:

```{r}
# Sequence creation (steps of 1)
x <- 1:10;
...
fibonacci <- function(n, full=FALSE) {
  ...
  # Default behavior: return just the n-th value
  if (!full) {
    return(fibvals[n])
  } 
...
y <- rnorm(1000, 0, 10) # (samples, mean, standard deviation)
plot(y)
...
plot(density(y)) #pdf of our y distribution
curve(dnorm(x, 0, 10), add=TRUE, lty=2, col="red") # my rnorm was (1000, 0, 10)
# My exponential distribution
plot(density(y_exp))
curve(dexp(x, 2), add=TRUE, lty=2, col="blue") # my rexp was (1000, 2)
...
Fy <- ecdf(y)
plot(y, Fy(y))
curve(pnorm(x, 0, 10), add=TRUE, lty=2, col="red")

Fy_exp <- ecdf(y_exp)
plot(y_exp, Fy(y_exp))
curve(pexp(x, 2), add=TRUE, lty=2, col="red")
```


## Q-Q Plots

Another tool to analyse and compare two distributions is through the use of Q-Q plots. Q stands for quantile, and the analysis consists on visually comparing the values of the same quantiles for two different sets of data. It compares the growth of two data sets. If the result is a line with zero offset and slope 1, then both sets have a similar shape, and therefore they probably follow the same random variable.
...
```{r}
# Q-Q plot generation # comparing the two functions value by value
...
If we only compare the cumulative distribution function, they may look similar:
```{r}
plot(ecdf(x))
...
more_1100 <- pkts[pkts>1100]; more_1100
plot(density(more_1100))
mean(more_1100)
```
